...
module Organizations
  ...
  module Callbacks
    ...
    #
    # @param event [Symbol] The event type (e.g., :organization_created, :member_added)
    ...
    def dispatch(event, **context_data)
      ...
      callback = config.public_send(:"after_#{event}")
    ...
    def execute_safely(callback, context)
      ...
      when 2
        # Support legacy 2-arg callbacks like ->(org, user) {}
        callback.call(context.organization, context.user)
      when 0
        callback.call
      else
        log_warn "[Organizations] Callback has unexpected arity (#{callback.arity}). Expected 0, 1, or 2."
      end
    rescue StandardError => e
      # Log but don't re-raise - callbacks should never break organization operations
      log_error "[Organizations] Callback error for #{context.event}: #{e.class}: #{e.message}"
