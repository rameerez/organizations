...
class JoinStateTest < ActiveSupport::TestCase
  ...
  test "accepts_domain_joining? tracks domains and unclaimed allowlist entries" do
    refute @org.accepts_domain_joining?

    @org.add_domain!("example.com")
    assert @org.accepts_domain_joining?
    ...
    refute @org.reload.accepts_domain_joining?, "claimed entries no longer open joining"
  end

  test "accepts_code_joining? counts only actually-redeemable codes" do
    refute @org.accepts_code_joining?

    code = @org.generate_join_code!(max_uses: 1)
    assert @org.accepts_code_joining?
    ...
    refute @org.accepts_code_joining?, "expired codes must not count"
    assert expired.expired?

    revoked = @org.generate_join_code!.tap(&:revoke!)
    refute @org.accepts_code_joining?, "revoked codes must not count"
